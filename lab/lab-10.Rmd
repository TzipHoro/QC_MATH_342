...
bills_payments_discounts$due_date = ymd(bills_payments_discounts$due_date)
bills_payments_discounts$invoice_date = ymd(bills_payments_discounts$invoice_date)
...
bills_data = bills_payments_discounts %>% 
  group_by(id) %>% 
  summarise(total_paid_amount = sum(paid_amount), 
            customer_id = first(customer_id), 
            discount_id = first(discount_id),
            num_days = first(num_days),
            total_amount = first(tot_amount),
            daysBtwn_invoice_due = as.numeric(first(due_date) - first(invoice_date))) %>%
  mutate(total_paid_amount = ifelse(is.na(total_paid_amount), 0, total_paid_amount), 
         paid_in_full = ifelse(total_paid_amount >= total_amount, 1, 0)) %>% 
  na.omit
bills_data
...
```{r}
random_indices = sample(1:nrow(bills_data))
X = bills_data[random_indices, c("total_amount", "num_days", "daysBtwn_invoice_due")]
y = bills_data[random_indices, "paid_in_full"]
...
tree_mod = YARFCART(X = X_train, y = y_train)
y_hat_train = predict(tree_mod, X_train)
y_hat_test = predict(tree_mod, X_test)

oos_conf_table = table(y_test, y_hat_test)
oos_conf_table
FDR = oos_conf_table[1, 2] / sum(oos_conf_table[ , 2])
FDR
FOR = oos_conf_table[2, 1] / sum(oss_conf_table[ , 1])
FOR
```
...
oos_se = sd(Boston_test$medv - yhat)
```
...
s_e_bag = c()
for (m in 1:M) {
  mod_bag = YARFBAG(Boston_train[, 1:13], Boston_train$medv, calculate_oob_error = FALSE)
  y_hat_test_bag = predict(mod_bag, Boston_test)
  s_e_bag = c(s_e_bag, sd(Boston_test$medv - y_hat_test_bag))
}
oos_se
mean(s_e_bag)
```
...
The oos standard error was reduced by approximately 1.
